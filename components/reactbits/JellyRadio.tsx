...
export default function JellyRadio({
  ...
  const at = current ? list.findIndex((it) => it.value === current) : -1;
  ...
  const apply = (sel: number, instant?: boolean) => {
    ...
    const hasSelection = sel >= 0 && sel < C.count;
    const push = hasSelection ? ((widths.current[sel] ?? 0) * C.swell) / 2 + C.barge : 0;
    for (let i = 0; i < C.count; i++) {
      const mv = mvFor(i);
      const on = i === sel;
      const far = hasSelection ? Math.abs(i - sel) : 1;
      const dir = hasSelection ? Math.sign(i - sel) * (rtl ? -1 : 1) : 0;
      const x = dir * push;
      const s = on ? 1 + C.swell : hasSelection ? 1 - C.shrink : 1;
      if (instant || C.reduce) {
        mv.x.jump(x);
        mv.sx.jump(s);
        mv.sy.jump(s);
